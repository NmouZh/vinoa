...
1. 入口形态：`vinoa init` 作用于当前目录，带 `<name>` 时新建子目录——是否照此？
2. 提问节奏：分三组（身份 / 平台与版本 / 可选项）并在组间可回退，还是一次问完？
3. 确认步骤：默认打印摘要 + Y/n，`--yes` 跳过——是否保留？
4. 完成提示：除路径/build/runServer 外，是否还要带版本注意事项与文档链接？
