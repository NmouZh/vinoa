...
/**
{% if is_lang_en %}
 * Command behaviour for the Sponge line (experimental skeleton).
 *
 * <p>It intentionally has no SpongeAPI type in its signature: the bootstrap registers it
 * against whichever API line the version matrix selected.
{% elif cap_dual_lang %}
 * Sponge 线的命令行为（实验性骨架）。
 *
 * <p>签名里刻意不出现 SpongeAPI 类型：由 bootstrap 针对版本矩阵选定的 API 线接入注册。
 * Command behaviour for the Sponge line (experimental skeleton). It intentionally has no
 * SpongeAPI type in its signature: the bootstrap registers it against whichever API line the
